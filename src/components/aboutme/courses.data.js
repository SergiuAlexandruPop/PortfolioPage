const COURSES_DATA = [
  {
    name: "Complete React Hooks Course 2020: A - Z ( Scratch to React )",
    url: "https://www.udemy.com/course/react-hooks-course",
    id: 1,
  },

  {
    name: "Complete React Developer in 2020 (w/ Redux, Hooks, GraphQL)",
    url:
      "https://www.udemy.com/course/complete-react-developer-zero-to-mastery/",
    id: 2,
  },
  {
    name: "The Modern React Bootcamp (Hooks, Context, NextJS, Router)",
    url: "https://www.udemy.com/course/modern-react-bootcamp/",
    id: 3,
  },
  {
    name: "Modern React with Redux {2020 Update}",
    url: "https://www.udemy.com/course/react-redux/",
    id: 4,
  },
  {
    name: "The Complete Web Developer in 2020: Zero to Mastery",
    url:
      "https://www.udemy.com/course/the-complete-web-developer-zero-to-mastery/",
    id: 5,
  },
  {
    name: "JavaScript, The Advanced Concepts",
    url: "https://www.udemy.com/course/advanced-javascript-concepts/",
    id: 6,
  },
  {
    name: "JavaScript, Understanding the Weird Parts",
    url: "https://www.udemy.com/course/understand-javascript/",
    id: 7,
  },
  {
    name: "The Complete JavaScript Course 2020: Build Real Projects!",
    url: "https://www.udemy.com/course/the-complete-javascript-course/",
    id: 8,
  },
  {
    name: "Advanced CSS and Sass: Flexbox, Grid, nmations and More!",
    url: "https://www.udemy.com/course/advanced-css-and-sass/",
    id: 9,
  },
  {
    name: "Build Responsive Real World Websites with HTML5 and CSS3",
    url:
      "https://www.udemy.com/course/design-and-develop-a-killer-website-with-html5-and-css3/",
    id: 10,
  },
  {
    name: "All Front-End content from FreeCodeCamp(not really a course)",
    url: "https://www.freecodecamp.org/learn",
    id: 11,
  },
];

export default COURSES_DATA;
